"""Unit test for the transform ned_to_aer function.
"""

import numpy as np
import pytest
from kinematics_library import KinematicVector
from kinematics_library.transforms import ned_to_aer
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            KinematicVector(3.665191434 - (2 * np.pi), -0.6981317, 70.0),
            1.0e-6,
            "ned_to_aer with KinematicVector",
        ),
    ],
)
def test_ned_to_aer_kv(kv, want, approx, desc):
    """Unit test for ned_to_aer function with."""
    out = ned_to_aer(kv)
...
def test_ned_to_aer_kv_with_out(kv, out, want, approx, desc):
    """Unit test for ned_to_aer function with out."""
    ned_to_aer(kv, out)
    assert out.x == pytest.approx(want[0], abs=approx), "x: " + desc
    assert out.y == pytest.approx(want[1], abs=approx), "y: " + desc
    assert out.z == pytest.approx(want[2], abs=approx), "z: " + desc
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            np.array((3.665191434 - (2 * np.pi), -0.6981317, 70.0)),
            1.0e-6,
            "ned_to_aer with 1 dimensional NumPy array",
        ),
    ],
)
def test_ned_to_aer_numpy_1d(kv, want, approx, desc):
    """Unit test for ned_to_aer function."""
    out = ned_to_aer(kv)
...
def test_ned_to_aer_numpy_1d_with_out(kv, out, want, approx, desc):
    """Unit test for ned_to_aer function with out."""
    ned_to_aer(kv, out)
    assert out.shape == kv.shape, "validate shape"
    assert out[0] == pytest.approx(want[0], abs=approx), "x: " + desc
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            np.array((3.665191434 - (2 * np.pi), -0.6981317, 70.0)).reshape(1, 3),
            1.0e-6,
            "ned_to_aer with 2 dimensional NumPy array",
        ),
        (
            np.array(
                (
            ...
            np.array(
                (
                    (3.665191434 - (2 * np.pi), -0.6981317, 70.0),
                    (3.665191434 - (2 * np.pi), -0.6981317, 70.0),
                    (3.665191434 - (2 * np.pi), -0.6981317, 70.0),
                )
            ),
            1.0e-6,
            "ned_to_aer with 2 dimensional NumPy array multiple rows",
        ),
    ],
)
def test_ned_to_aer_numpy_2d(kv, want, approx, desc):
    """Unit test for ned_to_aer function."""
    out = ned_to_aer(kv)
    assert out.shape == kv.shape, "validate shape"
...
def test_ned_to_aer_numpy_2d_with_out(kv, out, want, approx, desc):
    """Unit test for ned_to_aer function with out."""
    ned_to_aer(kv, out)
    assert out.shape == kv.shape, "validate shape"
    for i in range(kv.shape[0]):
