...
PyObject *ecef_to_ned(PyObject *self, PyObject *args, PyObject *kwargs)
{
    if (PyArray_API == NULL)
    {
        import_array();
    ...
    if (PyObject_TypeCheck(object, KinematicVectorTypeRef))
    {
        ...
        int error = transform_ecef_to_ned(&val->vector, &refPtr->vector, &result->vector);
    ...
    else if (PyArray_Check(object))
    {
        ...
        if (numDims == 1)
        {
            if (shape[0] == 3)
            {
                kvector_t ecef;
                kvector_t result;
                ecef.x = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR1(pyArray, 0));
                ecef.y = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR1(pyArray, 1));
                ecef.z = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR1(pyArray, 2));
                if (wasKVector == 0)
                {
                    PyArray_SetRef((PyArrayObject *)ref, &refLla, -1);
                }
                int error = transform_ecef_to_ned(&ecef, &refLla, &result);
                if (error != KL_NO_ERROR)
                {
                    if (output == NULL)
                    {
        ...
        else if (numDims == 2)
        {
            if (shape[1] == 3)
            {
                ...
                {
                    ecef.x = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR2(pyArray, i, 0));
                    ecef.y = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR2(pyArray, i, 1));
                    ecef.z = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR2(pyArray, i, 2));
                    if (wasKVector == 0)
                    {
