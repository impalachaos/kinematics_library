...
PyObject *ned_to_ecef(PyObject *self, PyObject *args, PyObject *kwargs)
{
    if (PyArray_API == NULL)
    {
        import_array();
    ...
    if (PyObject_TypeCheck(object, KinematicVectorTypeRef))
    {
        ...
        int error = transform_ned_to_ecef(&val->vector, &refPtr->vector, &result->vector);
    ...
    else if (PyArray_Check(object))
    {
        ...
        if (numDims == 1)
        {
            if (shape[0] == 3)
            {
                kvector_t ned;
                kvector_t result;
                ned.x = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR1(pyArray, 0));
                ned.y = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR1(pyArray, 1));
                ned.z = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR1(pyArray, 2));
                if (wasKVector == 0)
                {
                    PyArray_SetRef((PyArrayObject *)ref, &refLla, -1);
                }
                int error = transform_ned_to_ecef(&ned, &refLla, &result);
                if (error != KL_NO_ERROR)
                {
                    if (output == NULL)
                    {
        ...
        else if (numDims == 2)
        {
            if (shape[1] == 3)
            {
                ...
                {
                    ned.x = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR2(pyArray, i, 0));
                    ned.y = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR2(pyArray, i, 1));
                    ned.z = PyArray_ToPrecisionType(pyArray, PyArray_GETPTR2(pyArray, i, 2));
                    if (wasKVector == 0)
                    {
