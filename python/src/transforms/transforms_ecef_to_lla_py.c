...
PyObject *ecef_to_lla(PyObject *self, PyObject *args, PyObject *kwargs)
{
    if (PyArray_API == NULL)
    {
        import_array();
    ...
    if (PyObject_TypeCheck(object, KinematicVectorTypeRef))
    {
        ...
        int error = transform_ecef_to_lla(&val->vector, &result->vector);
    ...
    else if (PyArray_Check(object))
    {
        ...
        if (numDims == 1)
        {
            if (shape[0] == 3)
            {
                ...
                int error = transform_ecef_to_lla(&ecef, &result);
                if (error != KL_NO_ERROR)
                {
                    if (output == NULL)
                    {
